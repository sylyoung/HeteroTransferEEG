...
# @Author  : Siyang Li
# @File    : sda-odl.py
# He et al. - 2018 - Triplet-Center Loss for Multi-view 3D Object Retrieval
# implemented myself
import numpy as np
import argparse
import os
...
from utils.loss import tcl_loss
...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        tcl_loss_src = tcl_loss(features_source, labels_source, num_classes=args.class_num, margin=args.margin, args=args)
        tcl_loss_tgt = tcl_loss(features_tgttrain, labels_tgttrain, num_classes=args.class_num, margin=args.margin, args=args)

        loss = (source_classifier_loss + tgttrain_classifier_loss) / 2 + (
                    (tcl_loss_src + tcl_loss_tgt) / 2) * args.tcl_weight
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            b = ((tcl_loss_src + tcl_loss_tgt) / 2).item() * args.alignment_weight
            print("CLS {:.3f} TCL {:.3f}".format(a, b))
            base_network.eval()
...
if __name__ == '__main__':
    ...
    for data_name in data_name_list:
        ...
        args.method = 'SDA-TCL-' + str(args.num_target_train)
