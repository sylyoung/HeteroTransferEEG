...
# @Author  : Siyang Li
# @File    : sda-odl.py
# Liu et al. - 2022 - Orientational Distribution Learning With Hierarchical Spatial Attention for Open Set Recognition
# implemented myself and adapted a bit
import numpy as np
import argparse
import os
import torch
...
from utils.loss import odl_loss, datcl_loss
...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        if len(torch.unique(labels_tgttrain)) != args.class_num:
            continue

        if len(torch.unique(labels_source)) != args.class_num:
            continue

        intra_loss_src, inter_loss_src = odl_loss(features_source, labels_source, num_classes=args.class_num, margin=args.m_margin, args=args, orientation=True)
        intra_loss_tgt, inter_loss_tgt = odl_loss(features_tgttrain, labels_tgttrain, num_classes=args.class_num, margin=args.m_margin, args=args, orientation=True)
        loss = (source_classifier_loss + tgttrain_classifier_loss) / 2 + \
               (intra_loss_src + intra_loss_tgt) * args.intra_weight / 2 + \
               (inter_loss_src + inter_loss_tgt) * args.inter_weight / 2
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            a = ((source_classifier_loss + tgttrain_classifier_loss) / 2).item()
            b = ((intra_loss_src + intra_loss_tgt) / 2).item() * args.intra_weight
            c = ((inter_loss_src + inter_loss_tgt) / 2).item() * args.inter_weight
            print("CLS {:.3f} Intra {:.3f} Inter {:.3f}".format(a, b, c))
            base_network.eval()
...
if __name__ == '__main__':
    ...
    for data_name in data_name_list:
        ...
        args.method = 'SDA-ODL-' + str(args.num_target_train)
        ...
        args.intra_weight = 0.1

        args.inter_weight = 0.1
