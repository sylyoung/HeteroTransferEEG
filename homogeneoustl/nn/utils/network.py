...
# @Author  : Siyang Li
# @File    : network.py
# credit: code snippet originally written by Wen Zhang, also partially adapted from https://github.com/thuml/Transfer-Learning-Library
import numpy as np
import torch as tr
...
from models.EEGNet import EEGNet_feature, EEGNet, EEGNet_feature_ld, EEGNet_feature_map, EEGNet_BN_feature, EEGNet_feature_beforedropout
from models.FC import FC, FC_xy, FC_xy_flat, FC_xy_dropout
...
def backbone_net(args, return_type='y'):
    ...
    if args.backbone == 'EEGNet':
        netF = EEGNet_feature(n_classes=args.class_num,
                            ...
                            norm_rate=0.5)
    if return_type == 'y':
        if 'reduce' in args.method:
            netC = FC(2, args.class_num)
        else:
            netC = FC(args.feature_deep_dim, args.class_num)
    elif return_type == 'xy':
        if 'reduce' in args.method:
            netC = FC_xy(2, args.class_num)
        else:
