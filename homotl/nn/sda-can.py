...
# @Author  : Siyang Li
# @File    : sda-can.py
# Kang et al. - 2019 - Contrastive Adaptation Network for Unsupervised Domain Adaptation
# implemented myself
import numpy as np
import argparse
import os
...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        assert args.class_num == 2, print('only implemented binary case')
        ...
        ########################Same Class Cross Domain Alignment####################

        for i in range(args.class_num):
            inds_src = torch.where(labels_source == i)[0]
            features_class_src = features_source[inds_src]
            inds_tgt = torch.where(labels_tgttrain == i)[0]
            features_class_tgt = features_tgttrain[inds_tgt]
            if len(inds_src) != len(inds_tgt):
                lens = np.min((len(inds_src), len(inds_tgt)))
                if lens == len(inds_src):
                    features_class_tgt = features_class_tgt[:lens]
                else:
                    features_class_src = features_class_src[:lens]
            alignment_loss += mkmmd(features_class_src, features_class_tgt)
        ########################Same Class Cross Domain Alignment####################

        ########################Different Class Cross Domain Seperation (as negative alignment)####################

        inds_src = torch.where(labels_source == 0)[0]
        features_class_src = features_source[inds_src]
        inds_tgt = torch.where(labels_tgttrain == 1)[0]
        features_class_tgt = features_tgttrain[inds_tgt]
        if len(inds_src) != len(inds_tgt):
            lens = np.min((len(inds_src), len(inds_tgt)))
            if lens == len(inds_src):
        ...
        inds_src = torch.where(labels_source == 1)[0]
        features_class_src = features_source[inds_src]
        inds_tgt = torch.where(labels_tgttrain == 0)[0]
        features_class_tgt = features_tgttrain[inds_tgt]
        if len(inds_src) != len(inds_tgt):
            lens = np.min((len(inds_src), len(inds_tgt)))
            if lens == len(inds_src):
