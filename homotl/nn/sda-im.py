...
# @Author  : Siyang Li
# @File    : sda-im.py
# Liang et al. - 2021 - Source Data-Absent Unsupervised Domain Adaptation Through Hypothesis Transfer and Labeling Transfer
# adapted from its original implementation, and also by Changming Zhao
# Note that SHOT-IM was not originally supposed to apply to target labeled data
import numpy as np
import argparse
import os
import torch
...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        outputs_tgttrain = outputs_tgttrain.float().cpu()

        args.epsilon = 1e-5
        softmax_out = nn.Softmax(dim=1)(outputs_tgttrain)
        ent_loss = torch.mean(Entropy(softmax_out))
        msoftmax = softmax_out.mean(dim=0)
        reg_loss = torch.sum(msoftmax * torch.log(msoftmax + args.epsilon))
        uncertainty_loss = ent_loss + reg_loss

        loss = (source_classifier_loss + tgttrain_classifier_loss) / 2 + uncertainty_loss * args.uncertainty_weight
...
if __name__ == '__main__':
    ...
    for data_name in data_name_list:
        ...
        args.method = 'SDA-IM-' + str(args.num_target_train)
