...
# @Author  : Siyang Li
# @File    : sda-mstn.py
# Xie et al. - Learning Semantic Representations for Unsupervised Domain Adaptation
# from original author implementation
import numpy as np
import argparse
import os
import torch
...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        mstn = MSTN(n_class=args.class_num, deep_feature_dim=args.feature_deep_dim, cuda=args.data_env != 'local')
        _, _, alignment_loss = mstn.adloss(None, None, features_source, features_tgttrain, labels_source, labels_tgttrain)
        alignment_loss *= 100

        # moving average
        den = 1.0 + math.exp(-10 * (iter_num / max_iter))
        lamb = 2.0 / den - 1.0

        loss = (source_classifier_loss + tgttrain_classifier_loss) / 2 + alignment_loss * args.alignment_weight * lamb
...
if __name__ == '__main__':
    ...
    for data_name in data_name_list:
        ...
        args.method = 'SDA-MSTN-' + str(args.num_target_train)
