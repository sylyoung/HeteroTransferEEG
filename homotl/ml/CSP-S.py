...
# @Author  : Siyang Li
# @File    : CSP-S.py
# CSP-LDA for cross-subject analysis (only using source labeled data)
import mne
import numpy as np
import torch
...
def ml_cross(dataset, info, align, approach, calbr_n):
    ...
    for i in range(num_subjects):
        ...
        if paradigm == 'MI':
            ...
            # source CSP
            source_csp = mne.decoding.CSP(n_components=6)

            source_x_csp = source_csp.fit_transform(source_x, source_y)

            test_x_csp = source_csp.transform(test_x)

            pred, model = ml_classifier(approach, False, source_x_csp, source_y, test_x_csp, return_model=True)
